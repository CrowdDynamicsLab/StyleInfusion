...
from models.models import StyleClassifier, SiameseStyleClassifier, StyleClassifierScore

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print('Training on: ', device)


def calculate_accuracy(test_labels, test_outputs):
    return np.sum(test_labels == test_outputs) / len(test_labels)
...
def fine_tune(args):
    ...
    train_dataloader, test_dataloader = get_data_loaders(
        args, tokenizer, dataset_type=dataset_type, data_loader_fn=data_loader_fn, train_dir=args.train_data_dir, test_dir=args.test_data_dir)
    ...
    # load model
    model = torch.load(
        args.trained_model_path) if args.trained_model_path and args.load_model else model
    ...
    for epoch in range(args.epochs):
        if args.do_train:
            training(args, model, train_dataloader, optimizer,
                     lr_scheduler, loss_fn, epoch)
        if args.do_eval:
            evaluation(args, model, test_dataloader, tokenizer,
                       show_incorrect=args.show_incorrect)

    torch.save(model, args.model_save_name)
    if args.save_model:
        # 
        torch.save({'state_dict' : model.state_dict(), 'use_se':True}, args.model_save_name.replace('.pt', '_sd.pt'))


def training(args, model, train_dataloader, optimizer, lr_scheduler, loss_fn, epoch):
    model.train()
    for _, data in enumerate(train_dataloader):
        if args.architecture.lower() == "siamese":
            ...
            label = data['label'].to(device, dtype=torch.long)
            confidence = data['confidence'].to(device, dtype=torch.float)
            label = torch.where(label != 0, label, torch.ones_like(label) * -1)
            label = 0.5 + (label * (confidence/2))

            outputs = model(ids, mask, token_type_ids)
            # convert to 0/1 preds
            # outputs = (outputs > 0.5).to(torch.float)
...
def evaluation(args, model, test_dataloader, tokenizer, show_incorrect=False):
    model.eval()
    ...
    with torch.no_grad():
        for _, data in enumerate(test_dataloader):
            ...
            preds = (outputs > 0.5).to(torch.float).cpu().detach().numpy()
            ...
            if show_incorrect:
                ...
                for i, pred in enumerate(correct_preds):
                    if not pred:
                        print(f'Model Confidence: {outputs[i] / torch.sum(outputs).item()}')
                        if args.architecture.lower() == "siamese":
